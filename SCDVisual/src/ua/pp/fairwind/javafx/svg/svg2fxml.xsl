<xsl:transform xmlns:xsl="http://www.w3.org/1999/XSL/Transform"
    ...
    <xsl:template match="/">
        ...
        <Group xmlns:fx="http://javafx.com/fxml" id="Document">
            <children>
                <xsl:apply-templates/>
            </children>
        </Group>
            ...
            <xsl:for-each select="str:split($transform, ';')">
                ...
                <xsl:if test="substring-before(.,'(')='matrix'">
                    <xsl:variable name="coordinates"
                                  select="str:split(substring-before(substring-after(.,'matrix('),')'),',')"/>
                ...
                <xsl:when test="contains($style,'opacity:')">
                    <xsl:choose>
                        <xsl:when test="contains(substring-after($style,'opacity:'),';')">
                            <xsl:value-of
                                    select="substring-before(substring-after($style,'opacity:'),';')"></xsl:value-of>
                        </xsl:when>
                        <xsl:otherwise>
                                ...
                                <opacity>
                                    <xsl:choose>
                                        <xsl:when test="contains(substring-after($style,'stroke-opacity:'),';')">
                                            <xsl:value-of
                                                    select="number(substring-before(substring-after($style,'stroke-opacity:'),';')) * number($opacity)"/>
                                        </xsl:when>
                                        <xsl:otherwise>
                                            <xsl:value-of
                                                    select="substring-after($style,'stroke-opacity:')"></xsl:value-of>
                                        </xsl:otherwise>
                                    </xsl:choose>
                            ...
                            <opacity>
                                <xsl:choose>
                                    <xsl:when test="contains($style,'fill-opacity')">
                                        <xsl:value-of
                                                select="number(substring-before(substring-after($style,'fill-opacity:'),';')) * number($opacity)"/>
                                    </xsl:when>
                                    <xsl:otherwise>
                                        <xsl:value-of select="$opacity"/>
                                    </xsl:otherwise>
                                        ...
                                        <opacity>
                                            <xsl:choose>
                                                <xsl:when test="contains(substring-after(@style,'stop-opacity:'),';')">
                                                    <xsl:value-of
                                                            select="substring-before(substring-after(@style,'stop-opacity:'),';')"></xsl:value-of>
                                                </xsl:when>
                                                <xsl:otherwise>
                                                    <xsl:value-of
                                                            select="substring-after(@style,'stop-opacity:')"></xsl:value-of>
                                                </xsl:otherwise>
                                            </xsl:choose>
                                        </opacity>
                                    </xsl:when>
                ...
                <xsl:when test="$gradient/@gradientTransform !=''">
                    <xsl:choose>
                        ...
                        <xsl:when test="contains($gradient/@gradientTransform,'matrix(')">
                            <xsl:value-of
                                    select="substring-before( substring-after($gradient/@gradientTransform,'matrix('),')')"/>
                        </xsl:when>
                    </xsl:choose>
                </xsl:when>
                <xsl:otherwise>
            ...
            <xsl:when test="$hexNumber">
                <xsl:call-template name="HexToDecimal">
                    <xsl:with-param name="decimalNumber"
                                    select="($decimalNumber*16)+number(substring-before(substring-after('00/11/22/33/44/55/66/77/88/99/A10/B11/C12/D13/E14/F15/a10/b11/c12/d13/e14/f15/',substring($hexNumber,1,1)),'/'))"/>
