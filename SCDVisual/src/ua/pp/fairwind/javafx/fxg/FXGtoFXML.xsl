<xsl:transform xmlns:xsl="http://www.w3.org/1999/XSL/Transform" xmlns:fxg="http://ns.adobe.com/fxg/2008" version="1.0">
    ...
    <xsl:template match="fxg:Group">
        <Group>
            <xsl:if test="@x != ''">
                <xsl:attribute name="translateX">
                    <xsl:value-of select="attribute::x"/>
                </xsl:attribute>
            </xsl:if>
            <xsl:if test="@y != ''">
                <xsl:attribute name="translateY">
                    <xsl:value-of select="attribute::y"/>
                </xsl:attribute>
            </xsl:if>
            <xsl:if test="@visible != ''">
                <xsl:attribute name="visible">
                    <xsl:value-of select="attribute::visible"/>
                </xsl:attribute>
            </xsl:if>
            <xsl:if test="@alpha != ''">
                <xsl:attribute name="opacity">
                    <xsl:value-of select="attribute::alpha"/>
                </xsl:attribute>
            </xsl:if>
            <xsl:if test="@scaleX != ''">
                <xsl:attribute name="scaleX">
                    <xsl:value-of select="attribute::scaleX"/>
                </xsl:attribute>
            </xsl:if>
            <xsl:if test="@scaleY != ''">
                <xsl:attribute name="scaleY">
                    <xsl:value-of select="attribute::scaleY"/>
                </xsl:attribute>
            </xsl:if>
            <children>
                <xsl:apply-templates/>
    ...
    <xsl:template match="fxg:BitmapGraphic">
        <ImageView>
            ...
            <xsl:choose>
                <xsl:when test="@scaleX != '' and @width !=''">
                    <xsl:attribute name="fitWidth">
                        <xsl:value-of select="attribute::width * attribute::scaleX"/>
                    </xsl:attribute>
                </xsl:when>
                <xsl:when test="@width != ''">
                    <xsl:attribute name="fitWidth">
            ...
            <xsl:choose>
                <xsl:when test="@scaleY != '' and @height !=''">
                    <xsl:attribute name="fitHeight">
                        <xsl:value-of select="attribute::height * attribute::scaleY"/>
                    </xsl:attribute>
                </xsl:when>
            ...
            <image>
                <Image url="@{substring-before(substring(attribute::source,9),&quot;&apos;&quot;)}"/>
            </image>
        </ImageView>
    </xsl:template>
    ...
    <xsl:template match="fxg:Path">
        <SVGPath content="{attribute::data}">
            ...
            <xsl:if test="@blendMode != ''">
                <xsl:attribute name="blendMode">
                    <xsl:value-of select="attribute::blendMode"/>
                </xsl:attribute>
            </xsl:if>
            <xsl:apply-templates/>
        </SVGPath>
        ...
        <Ellipse>
            <xsl:if test="@x != ''">
                <xsl:attribute name="centerX">
                    <xsl:value-of select="attribute::x + (attribute::width * 0.5)"/>
                </xsl:attribute>
            </xsl:if>
            <xsl:if test="@y != ''">
                <xsl:attribute name="centerY">
                    <xsl:value-of select="attribute::y + (attribute::height * 0.5)"/>
                </xsl:attribute>
            </xsl:if>
            <xsl:if test="@width != ''">
                <xsl:attribute name="radiusX">
                    <xsl:value-of select="attribute::width * 0.5"/>
                </xsl:attribute>
            </xsl:if>
            <xsl:if test="@height != ''">
                <xsl:attribute name="radiusY">
                    <xsl:value-of select="attribute::height * 0.5"/>
                </xsl:attribute>
            </xsl:if>
            <xsl:apply-templates/>
        </Ellipse>
    ...
    <xsl:template match="fxg:fill/fxg:RadialGradient">
        <fill>
            <RadialGradient centerX="{attribute::x}" centerY="{attribute::y}" radius="{attribute::scaleX}"
                            proportional="false">
                <stops>
                    <xsl:apply-templates/>
                </stops>
    ...
    <xsl:template match="fxg:fill/fxg:LinearGradient">
        <fill>
            <LinearGradient startX="{attribute::x}" startY="{attribute::y}" endX="{attribute::scaleX}"
                            proportional="false">
                <xsl:choose>
                    <xsl:when test="@scaleY != ''">
