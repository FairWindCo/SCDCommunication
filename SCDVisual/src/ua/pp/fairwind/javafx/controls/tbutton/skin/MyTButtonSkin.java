package ua.pp.fairwind.javafx.controls.tbutton.skin;
...
import ua.pp.fairwind.javafx.controls.tbutton.MyTButton;
...
public class MyTButtonSkin extends SkinBase<MyTButton> implements Skin<MyTButton> {
    ...
    // ******************** Constructors **************************************
    public MyTButtonSkin(final MyTButton CONTROL) {
        super(CONTROL);
        pane = new Pane();
        init();
        initGraphics();
    ...
    private void initGraphics() {
        ...
        on.setEffect(offDropShadow);
        on.setVisible(getSkinnable().isSelected());

        off.setEffect(onDropShadow);
    ...
    protected void handleControlPropertyChanged(final String PROPERTY) {
        if ("RESIZE".equals(PROPERTY)) {
            ...
            getSkinnable().fireSelectEvent(getSkinnable().isSelected() ? new MyTButton.SelectEvent(getSkinnable(), getSkinnable(), MyTButton.SelectEvent.SELECT) : new MyTButton.SelectEvent(getSkinnable(), getSkinnable(), MyTButton.SelectEvent.DESELECT));
            on.setVisible(getSkinnable().isSelected());
            ledOn.setVisible(getSkinnable().isSelected());
    ...
    private void resize() {
        ...
        if (width > 0 && height > 0) {
            ...
            ledOff.setTranslateY(0.2291666666666666 * height);
            ...
            ledOn.setTranslateY(0.25 * height);
