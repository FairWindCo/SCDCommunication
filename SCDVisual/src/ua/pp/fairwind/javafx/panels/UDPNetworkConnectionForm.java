...
public class UDPNetworkConnectionForm  extends SimpleView {
	Line comline;
	StringProperty address=new SimpleStringProperty();
	IntegerProperty port=new SimpleIntegerProperty();
	UDPCommunicationLineAdapter adapter=null;
	
	public UDPNetworkConnectionForm(String title, Image icon,
	...
	public UDPNetworkConnectionForm(String title,Line comline) {
	...
	public void onShow(MenuExecutor executr) {
		if(comline!=null){
			if(comline.getCommunicationAdapter() instanceof UDPCommunicationLineAdapter){
				adapter=(UDPCommunicationLineAdapter) comline.getCommunicationAdapter();
	...
	protected Node createView() {
		...
		ok.onActionProperty().setValue(arg0 -> {
            if(comline!=null){
                ...
                adapter=new UDPCommunicationLineAdapter(address.getValue(), port.intValue());
